...
k_prod_PTHg =  4.192e-002, # PTHg synthesis rate (in microM.min^-1)
...
gamma_PO4_conv = 2e-001, # Sensitivity of vitamin D3 production to PO_4 (in mM). In the paper take 1/gamma_P_conv to find K_conv_PO4
...
n_prod_FGF = 5,
K_prod_PO4 = 1.6e000, # Sensitivity of FGF23 synthesis to PO4 (in mM)
...
n_abs = 2e000, # 
I_PO4 = 1.55e-003, # Phosphate intake (in micromol.min^-1)
...
k_pf_Ca = 4.4e-001, # Rate constant of Ca2+ transfer from plasma to fast bone pool (in min^-1)
k_fp_Ca = 2.34e-003, # Rate constant of Ca2+ transfer from fast bone pool to plasma (in min^-1)
k_pf_PO4 = 13.5e-000, # Rate constant of PO_4 transfer from plasma to fast bone pool (in min^-1)
k_fp_PO4 = 2.5165e-001, # Rate constant of PO_4 transfer from fast bone pool to plasma (in min^-1)
...
K_PT_PO4 = 1.6e000, # Sensitivity  of PO_4 reabsorption in PT to PO_4 (in mM)
...
Lambda_ac_P <- parameters["Ca_P_stoech"]*parameters["Lambda_ac_Ca"] # PO4 accretion rate constant (in min^-1)
r <- 10^(parameters["pH"] - parameters["pKa"]) # ratio depending on pH
a <- r/(1+r)
b <- a/r

parameters <- c(parameters, Lambda_ac_P, r, a, b)